...
resource "local_file" "rendered_cypress_test" {
  content = templatefile("${path.module}/cypress-test-template.cy.js.tpl", {
    website_address = var.environment == "prod" ? "${var.domain}" : "http://${google_compute_global_address.https_public_ip.address}"  #tests an ip or the actual domain depending on prod or not
